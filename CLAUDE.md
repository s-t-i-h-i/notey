...
- **Gestures**: single finger/Pencil draws, two fingers scroll (pan `minimumNumberOfTouches = 2`), pinch zooms. Custom recognizers coexist with PencilKit's; pan hit-testing must use the touch-down point (`startPagePoint`) because pans recognize ~10 pt late.
